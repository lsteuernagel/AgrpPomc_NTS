...
overview_umap = DimPlot(AgrpPomcNTS_withIEG,group.by = "celltype_annotation",label=TRUE,label.size = 5,repel = TRUE,
                        cols = getOkabeItoPalette(n = nannos),raster = TRUE,raster.dpi = c(1536,1536),pt.size = 1.4)+NoLegend()+NoAxes()+ggtitle(NULL)
overview_umap
...
p <- FeaturePlot(AgrpPomcNTS_withIEG,features = gene_set_to_plot,keep.scale = "all", combine = FALSE,raster = TRUE,order=TRUE,raster.dpi = c(rasterize_px,rasterize_px),
...
rasterize_px = 1536
seurat_pt_size = 1.9
gene_set_to_plot = c("Slc17a6","Slc32a1")
p <- FeaturePlot(AgrpPomcNTS_withIEG_neurons,features = gene_set_to_plot,keep.scale = "all", combine = FALSE,raster = TRUE,order=TRUE,raster.dpi = c(rasterize_px,rasterize_px),reduction="umap_neurons",
...
all_markers_per_cluster =  marker_genes %>%
  dplyr::filter(cluster %in% "82") %>%
  dplyr::group_by(cluster) %>%
  dplyr::filter(specificity > 2 & p_val_adj < 0.01) %>%
  dplyr::arrange(desc(specificity))

data.table::fwrite(all_markers_per_cluster,file = paste0(figure_path,"tal1_82_cluster_markers.txt"),sep="\t")
...
data.table::fwrite(th_cluster_info,file = paste0(figure_path,"th_cluster_info.txt"),sep="\t")
